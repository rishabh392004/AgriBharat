  ...
  async diagnose(input: DiagnosisRequest): Promise<DiagnosisResult> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (
          res.status === 400 ||
          errorText.includes("INVALID_FOLIAGE_DETECTED") ||
          errorText.includes("No genuine crop leaf")
        ) {
          throw new AppError(
            "INVALID_FOLIAGE_DETECTED: No genuine crop leaf detected in the image. Please re-capture a clear photo of the crop leaf.",
            400
          );
        }
        throw new Error(
      ...
      const precautions =
        Array.isArray(sol.precautions) && sol.precautions.length > 0
          ? sol.precautions
          : [
